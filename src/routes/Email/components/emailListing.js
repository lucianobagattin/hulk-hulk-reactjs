...
class EmailListing extends Component {
	...
	deleteEmail() {
		if (this.props.match.path === '/app/email/folder/trash') {
			this.props.onDeleteEmailFromTrash();
		} else {
	...
	spamEmail() {
		this.props.onSpamEmail();
	}

	draftEmail() {
		this.props.onDraftEmail();
	}

	render() {
		const { emails, selectedEmails, searchEmailText } = this.props;
									...
									<Box display="inline-flex" alignItems="center">
										<Box mr={{xs:'3px', md:1}}>
											<Tooltip title="Archive" placement="bottom">
												<Button onClick={() => this.draftEmail()} className={classes.secBtn}>
										...
										<Box mr={{xs:'3px', md:1}}>
											<Tooltip title="Mark As Spam" placement="bottom">
												<Button onClick={() => this.spamEmail()} className={classes.secBtn}>
										...
										<Box>
											<Tooltip title="Delete" placement="bottom">
												<Button onClick={() => this.deleteEmail()} className={classes.secBtn}>
						...
						<List className={`email-list-wrap ${classes.root}`}>
							{emails.map((email, index) => (
								<ListItem
									...
									key={index}
									onClick={() => this.readEmail(email)}
								>
									<ListItemIcon className={classes.check}>
										<Checkbox
											checked={email.selected}
											onClick={(e) => this.onSelectEmail(e, email)}
											id={'cb-' + index}
											color="primary"
...
export default withRouter(connect(mapStateToProps, {
	...
	getUnselectedAllEMails,
	onDeleteEmailFromTrash
})(withStyles(styles)(EmailListing)));