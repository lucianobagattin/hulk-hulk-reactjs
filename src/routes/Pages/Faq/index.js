...
export default function FAQ() {
	...
	const [expanded, setExpanded] = React.useState('generalEnquries_1');
	const [faqData] = React.useState(faq);
                           ...
                           <Box mb={2}><Typography variant="h6"><IntlMessages id={cat.category} /> </Typography></Box>
									{
										cat.content.map((faq, i) => (
											<Accordion className={classes.faqListItem} key={i} expanded={expanded === cat.id + '_' + faq.subid} onChange={handleChange(cat.id + '_' + faq.subid)}>
												<AccordionSummary
